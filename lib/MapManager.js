...
MapManager.prototype.storeMap = function(name, callback) {
	try {
		...
		const currentOpts = {
			...
			goto_timers: this.configuration.get("goto_timers"),
			afterCleanDest: this.configuration.get("system").afterCleanDest,
			mapRestoreType: this.configuration.get("system").mapRestoreType,
			autoMapRestore: this.configuration.get("system").autoMapRestore,
            autoDockReturn: this.configuration.get("system").autoDockReturn
		};
...
MapManager.prototype.loadMap = function(name, callback) {
	...
	.then(res => new Promise((resolve,reject) => {
		...
		if (fs.existsSync("/mnt/data/valetudo/maps/" + name + "/valetudo.json")) {
			...
			if (loadedOpts.goto_timers) {
				this.configuration.set("goto_timers",loadedOpts.goto_timers);
			}
			this.createInstance("cron", (loadedOpts.ztimers && loadedOpts.ztimers.length && loadedOpts.ztimers.some(zt => zt[0])) || (loadedOpts.goto_timers && loadedOpts.goto_timers.length && loadedOpts.goto_timers.some(gt => gt[0])));
			...
			if (loadedOpts.mapRestoreType) {
				this.configuration.set("system",Object.assign(systemOpts,{mapRestoreType: loadedOpts.mapRestoreType}));
			} else {
				this.configuration.set("system",Object.assign(systemOpts,{mapRestoreType: undefined}));
			}
			if (loadedOpts.autoMapRestore) {
				this.configuration.set("system",Object.assign(systemOpts,{autoMapRestore: loadedOpts.autoMapRestore}));
			} else {
				this.configuration.set("system",Object.assign(systemOpts,{autoMapRestore: undefined}));
			}
			if (loadedOpts.autoDockReturn) {
				this.configuration.set("system",Object.assign(systemOpts,{autoDockReturn: loadedOpts.autoDockReturn}));
			} else {
				this.configuration.set("system",Object.assign(systemOpts,{autoDockReturn: undefined}));
			}
		}
...
MapManager.prototype.restoreTimers = function(loadedOpts,output) {
	...
	.then(res => {
		...
		loadedOpts.timers.forEach(t => {
			p = p.then(() => new Promise((res,rej) => {
				this.vacuum.addTimer({id: t[1].replace(/ /g,"\u{a0}"), cron: t[2], fanpower: +t[5], segments: t[3], iterations: +t[4]}, (err,data) => {
					if (err) {
						rej(err);
					} else if (!t[0]) {
