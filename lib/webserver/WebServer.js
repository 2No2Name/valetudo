...
const WebServer = function (valetudo) {
    ...
    this.app.get("/api/" + "goto_timers", function (req, res) {
        const timers = self.configuration.get("goto_timers");
        res.json(timers.map(timer => ({
                ...
                coordinates: timer[3],
                goToSpotName: timer[4],
                spotIsDock: timer[5]
            }
        )));
    });
    this.app.get("/api/" + "ztimers", function (req, res) {
        const timers = self.configuration.get("ztimers");
        res.json(timers.map(timer => ({
            enabled: timer[0],
            id: timer[1],
            cron: timer[2],
    ...
    this.app.put("/api/" + "goto_timers", function (req, res) {
        if (req.body.id && req.body.cron && Array.isArray(req.body.coordinates)) {
            // Todo: Extended validation
            const ntimer = [
                true,
            ...
            self.configuration.set("goto_timers", timers);
            res.status(201).json({message: "ok"});
	...
	timerGroups.forEach((timerGroup) => {
	    this.app.put("/api/" + timerGroup + "/:timerID", function (req, res) {
		    if (req.body && req.params.timerID !== undefined) {
			    const timers = self.configuration.get(timerGroup);
			    ...
			    if (idx < 0) {
				    ...
				    self.configuration.set(timerGroup, timers);
				    if (!self.cronScheduler) {
					    self.createInstance("cron");
				    }
				    res.status(201).json({message: "ok"});
	    ...
	    this.app.delete("/api/" + timerGroup + "/:timerID", function (req, res) {
		    const timers = self.configuration.get(timerGroup);
		    let idx = timers.findIndex(timer => timer[1] === req.params.timerID);
		    if (idx < 0) {
			    res.status(404).send("No timer with selected id.");
